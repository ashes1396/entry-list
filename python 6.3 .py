#changes:
# added errorlist
# duplicate entrieslist
#updated cleanuppost to return two values
#entrylist includes three arguments
# errors list prints out duplicates
...
# new function entrylist to check for duplicates/quotes

def entryList(cleanup, eList, errorList, post):
	#recall that cleanup returns two values, username and entry line
	#note entryList has four arguments. 
	uname, eline = cleanup
	if uname not in eList:
		#print "username not in elist"
		eList.append(uname)
		print eline
	else:
		if "quote" in post:
			errorList.append(["quote!",uname, eline])
		else:
			errorList.append([uname, eline])


#new function errors. takes errorlisttotal and checks whether it is empty or not
def errors(error):
	if len(error) == 0:
		return
	else:
...
def printAllEntries(page):
	...
	while True:
		post, postEnd = getNextTarget(page)
		if post:
			if "#entry" in post or "#Entry" in post or "#ENTRY" in post:
				entryList(cleanUpPost(post), eListTotal, errorListTotal, post)
			page = page[postEnd:]
		else:
			break
	errors(errorListTotal)

#errors list total included
